...
async def test_websocket_active_order_desync_reconnect_simulation(client: AsyncClient, conn):
    ...
    # We assert that ws_handler has zero interaction with OrderUseCases, proving architectural decoupling.
    from unittest.mock import patch

    with patch("backend.application.use_cases.orders.OrderUseCases") as mock_use_cases_class:
        ...
        # Verify that OrderUseCases was never even instantiated (zero database/order interactions from WS handler)
        mock_use_cases_class.assert_not_called()

    # 4. Reconnect Simulator: Client receives disconnect.
    # It reconnects with the SAME token & device_id, resetting sequence back to 1.
    # This verifies connection-level recovery: the underlying active order survives the transport reset.
